"""Fetch the holdings for one particular ETF and display it.
"""
...
def main():
    ...
    parser.add_argument('ticker',
                        help='The ticker symbol to download')
    parser.add_argument('issuer',
                        help='The issuer for the ETF symbol')
    ...
    # Fetch the holdings to refresh the file.
    db = database.Database(args.dbdir)
    driver, _ = updatedb.fetch_holdings(args.ticker, args.issuer, None, db, False, args)
    if driver:
        driver.close()

    # Get the latest file.
    filename = database.getlatest(db, args.ticker)

    # Parse the file.
    downloader = issuers.get(args.issuer)
    holdings = downloader.parse(filename)
    portfolio.check_holdings(holdings)
